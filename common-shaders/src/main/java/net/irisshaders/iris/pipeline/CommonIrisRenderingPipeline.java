...
public abstract class CommonIrisRenderingPipeline implements WorldRenderingPipeline, IrisRenderingPipeline, ShaderRenderingPipeline, RenderTargetStateListener {
    ...
    public CommonIrisRenderingPipeline(ProgramSet programSet) {
        ...
        this.renderTargets = new RenderTargets(mainFBWidth, mainFBHeight,
                mainFBDepthTextureId,
                mainFBDepthBufferVersion, depthBufferFormat, programSet.getPackDirectives().getRenderTargetDirectives().getRenderTargetSettings(), programSet.getPackDirectives());
        ...
        if (shadowRenderTargets != null) {
            ...
            if (programSet.getPackDirectives().getShadowDirectives().isShadowEnabled().orElse(true)) {
                this.shadowRenderer = createShadowRenderer(this, programSet.getShadow().orElse(null),
    ...
    @Nullable
    protected abstract CommonShadowRenderer createShadowRenderer(CommonIrisRenderingPipeline commonIrisRenderingPipeline, ProgramSource programSource, PackDirectives packDirectives, ShadowRenderTargets shadowRenderTargets, ShadowCompositeRenderer shadowCompositeRenderer, CustomUniforms customUniforms, boolean b);
    ...
    protected CompletableFuture<MCShaderInstance> createShader(String name, Optional<ProgramSource> source, ShaderKey key, Executor syncExecutor) throws IOException {
        if (!source.isPresent()) {
    ...
    protected abstract CompletableFuture<MCShaderInstance> createShader(String name, Executor syncExecutor, ProgramSource source, ProgramId programId, AlphaTest fallbackAlpha,
    ...
    protected abstract CompletableFuture<MCShaderInstance> createShadowShader(String name, Optional<ProgramSource> source, ShaderKey key, Executor syncExecutor) throws IOException;
    protected abstract MCShaderInstance createFallbackShadowShader(String name, ShaderKey key) throws IOException;
    protected abstract CompletableFuture<MCShaderInstance> createShadowShader(String name, Executor syncExecutor, ProgramSource source, ProgramId programId, AlphaTest fallbackAlpha,
