...
public class BiomeColorCache {
    ...
    public BiomeColorCache(BiomeColorHelper.ColorResolver resolver, WorldSlice slice) {
        ...
        this.slice = slice;
        this.radius = 3; // TODO make configurable

        var origin = this.slice.getOrigin();

        this.minX = origin.minX() - (this.radius + 2);
        this.minZ = origin.minZ() - (this.radius + 2);

        this.height = origin.minY();
        this.dim = 16 + ((this.radius + 2) * 2);

        this.blendedColorsMinX = origin.minX() - 2;
        this.blendedColorsMinZ = origin.minZ() - 2;
