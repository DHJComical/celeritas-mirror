    ...
    public static void writeQuadVertices(VertexBufferWriter writer, PoseStack.Pose matrices, ModelQuadView quad, int color, int light, int overlay) {
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                ModelVertex.write(ptr, xt, yt, zt, multARGBInts(quad.getColor(i), color), quad.getTexU(i), quad.getTexV(i), overlay, ModelQuadUtil.mergeBakedLight(quad.getLight(i), quad.getVanillaLightEmission(), light), mergeNormalAndMult(quad.getForgeNormal(i), normal, matNormal));
